...
def process_frames(source_img, frame_paths, progress=None):
    ...
    for frame_path in frame_paths:
        ...
        frame = cv2.resize(frame, (0, 0), fx=0.5, fy=0.5)  # Reduce the resolution of the frame

        face_embeddings = get_face_embeddings(frame)
        if face_embeddings:
            for embedding in face_embeddings:
                target_face = get_face_from_embedding(embedding)
                target_frame = swap_face_in_frame(source_embedding, target_face, frame)

        try:
            cv2.imwrite(frame_path, target_frame)
